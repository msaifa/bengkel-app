...
- Minimalist & Professional: The UI is strictly **emoji-free**. It relies entirely on typography, spacing, and subtle colors rather than decorative emojis or complex icons.
...
### Data Formatting
- **Compact Currency**: In tight spaces (like 2-column mobile grids), large currency values must be abbreviated using `rb` (ribuan) and `jt` (jutaan) suffixes (e.g., Rp 70.000 → Rp 70rb, Rp 2.500.000 → Rp 2.5jt) to maintain the minimalist, single-line aesthetic.
...
- **Quick Stats Grid:** A tight `1fr 1fr` grid (`gap: 16px`) used for side-by-side data cards. This forces cards to remain on a single row even on mobile screens, requiring extreme layout brevity (Title, Value, Sub-value only).
...
### Decorative Depth
### Decorative Depth (Glassmorphism Blobs)
While the core system avoids heavy drop shadows, the dashboard introduces subtle depth and visual interest through **Decorative Gradient Shapes**:
- Soft, absolute-positioned radial gradients (e.g., `radial-gradient(circle, rgba(163,217,177,0.3) 0%, transparent 70%)`) placed behind card content.
- Cards must use `overflow: hidden` and `position: relative` to contain the blobs.
- These blobs are purely decorative (`pointer-events: none`, `z-index: 0`) and provide a subtle wash of color (blue, pink, green, red) to otherwise monochrome cream cards, adding a dynamic, premium feel without compromising the flat aesthetic.

Other depth comes entirely from the imagery itself:
...
**`data-card-minimal`** (Dashboard Quick Stats)
- Extremely breathable, text-only cards (no progress bars, no icons).
- Contains exactly 3 elements: Title (`{typography.caption-md}`, uppercase, `{colors.mute}`), Value (`{typography.heading-md}`), and Sub-value (`{typography.caption-sm}`).
- Features a soft decorative radial gradient blob in the corner for visual interest.
...
### Don't
- Don't use emojis for iconography or decoration. The UI must remain professional and rely on typography and spacing.
- Don't clutter cards with unnecessary text. If a card is part of a multi-column grid, strip it down to Title, Value, and Sub-value.
